          ...
          <div className="grid md:grid-cols-2 gap-6">
            {portfolioData.certificates.map((cert, i) => (
              <a href={cert.link} target="_blank" rel="noopener noreferrer" key={i}>
                <Card className="rounded-2xl border border-border bg-muted/20 p-8 hover:bg-card hover:border-primary/40 transition-all flex items-center gap-6 h-full group">
                  <div className="w-12 h-12 rounded-xl bg-background flex items-center justify-center text-3xl shadow-sm group-hover:scale-110 transition-transform">
                    🏅
                  </div>
                  <div className="space-y-2">
                    <h4 className="font-black text-xl text-foreground leading-tight tracking-tight group-hover:text-primary transition-colors">{cert.title}</h4>
                    <Badge variant="outline" className="text-[8px] font-black uppercase tracking-[0.2em] bg-primary/5 text-primary border-primary/20 flex w-fit items-center gap-1.5 px-2 py-0.5 rounded-md">
