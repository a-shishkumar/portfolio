  ...
  return (
    ...
    >
      <nav className={cn(
        ...
        scrolled ? "h-14 bg-background/80 backdrop-blur-2xl border border-white/5 shadow-[0_10px_40px_rgba(0,0,0,0.1)]" : "h-16 bg-transparent"
      )}>
        {/* Brand - Modern & Clean */}
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[7px] font-black uppercase tracking-[0.3em] text-muted-foreground mt-0.5 opacity-60">Architect</span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {dark ? <Sun className="w-4 h-4 text-primary" /> : <Moon className="w-4 h-4 text-muted-foreground group-hover:text-primary" />}
          </button>
        ...
        {menuOpen && (
          ...
          >
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={cn(
                    ...
                    pathname === link.href ? "bg-primary text-white shadow-lg shadow-primary/20" : "bg-muted/50 text-muted-foreground hover:bg-primary/5 hover:text-primary"
                  )}
                >
                  <div className={cn("w-1.5 h-1.5 rounded-full", pathname === link.href ? "bg-white" : "bg-primary/30")} />
                  {link.label}
                </Link>
              ))}
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-border/50">
              <Button className="w-full h-14" asChild onClick={() => setMenuOpen(false)}>
